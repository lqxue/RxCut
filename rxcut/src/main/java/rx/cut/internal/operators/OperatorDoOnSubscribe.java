...
/**
 * This operator modifies an {@link rx.cut.Observable} so a given action is invoked when the {@link rx.cut.Observable} is subscribed.
...
public class OperatorDoOnSubscribe<T> implements Operator<T, T> {
     ...
     * @param subscribe the action that gets invoked when the modified {@link rx.cut.Observable} is subscribed
     */
